# 上币评估系统 MVP 需求文档

> 目标：7天内交付可运行的端到端系统，路演时能现场跑出结果

---

## 核心原则

- 砍掉一切非必要功能，只保证核心链路跑通
- 不用付费 API，不用复杂后端
- Claude 是真正的分析引擎，不是摆设

---

## 系统流程

```
用户输入 Token 名称（如 SUI、NEAR、OP）
  ↓
自动拉取公开数据（3个稳定免费 API）
  ↓
Claude 综合分析 + 评分
  ↓
输出评估报告 + 雷达图
```

---

## 数据源（只用 CoinGecko，一个免费接口搞定所有）

| 数据源 | 提供数据 | API 状态 |
|--------|----------|----------|
| **CoinGecko** | 市值、交易量、价格、Twitter/Telegram 粉丝数、GitHub Star/提交数、已上线交易所列表 | 免费，无需 Key |

> 不接 Twitter API、不爬 Certik/Slowmist、不调 DeFiLlama、不调 GitHub API。
> CoinGecko 的 `developer_data` 字段包含 GitHub Star 数和近4周提交数，`tickers` 字段包含实时交易所上线情况，一个接口全部覆盖。
>
> 调用分两步：1) `/search?query=` 获取 coin_id；2) `/coins/{id}?tickers=true` 获取详情。
> tickers 数据传给 Claude 前需裁剪，只保留主流交易所名称，避免 prompt 超长。

---

## 评估维度（5个，精简版）

| 维度 | 权重 | 数据来源 | 评分范围 |
|------|------|----------|----------|
| 市场规模 | 30% | CoinGecko market_data | 0-30 |
| 社区活跃度 | 20% | CoinGecko community_data | 0-20 |
| 技术实力 | 20% | CoinGecko developer_data（Stars + 近4周提交数） | 0-20 |
| 竞争位置 | 15% | Claude 分析（基于 tickers 实时交易所列表，非 Claude 记忆） | 0-15 |
| 风险信号 | 15% | Claude 分析（基于全量数据综合判断） | 0-15 |

### 评分 → 推荐结论

| 总分 | 结论 |
|------|------|
| ≥ 75 | 强烈推荐上币 |
| 55-74 | 建议观望 |
| < 55 | 不建议 |

---

## Claude 的作用

Claude 不只是格式化输出，承担以下分析：

1. **竞争位置判断**：基于 tickers 实时交易所列表（由代码从 CoinGecko 提取并裁剪后传入），判断该项目在头部所的覆盖情况，不依赖 Claude 的训练集记忆
2. **语义层风险识别**：在规则引擎已判断的客观风险基础上，识别需要语义理解的额外风险（如赛道过度拥挤、项目定位模糊），最多额外扣 2 分（不让 Claude 判断需要历史数据的客观风险，如交易量暴增）
3. **自然语言解读**：将数字数据转化为"这意味着什么"的结论
4. **最终推荐理由**：给出 3 条核心支持理由 + 2 条主要风险点
5. **BYDFi 跟进紧迫性**：基于竞品已上线情况，给出 BYDFi 是否需要尽快跟进的判断（高/中/低）

---

## 技术栈

| 层 | 选型 | 理由 |
|----|------|------|
| 前端/界面 | Streamlit | 零前端工作量，Python 直出 UI |
| 数据处理 | Python + requests | 够用，不引入额外依赖 |
| AI 分析 | Claude API (claude-sonnet-4-6) | 主力引擎 |
| 可视化 | Plotly（雷达图） | Streamlit 原生支持 |
| 存储 | 无数据库，session 内存 | 砍掉 PostgreSQL/Redis |
| 历史记录 | 本地 JSON 文件（可选） | 够路演用 |

---

## 项目结构

```
token-assessment/
├── app.py                  # Streamlit 主入口
├── collectors/
│   └── coingecko.py        # CoinGecko 数据拉取（唯一数据源）
├── analyzer/
│   ├── scorer.py           # 各维度评分计算
│   └── claude_analyzer.py  # Claude API 调用
├── report/
│   └── chart.py            # 雷达图生成
├── .env                    # ANTHROPIC_API_KEY
└── requirements.txt
```

---

## 输出报告内容

路演时展示的报告包含：

1. **基础信息**：Token 名、市值、24h 交易量、当前价格
2. **各维度评分条**：5个维度得分可视化
3. **雷达图**：一眼看出强弱维度
4. **Claude 分析文字**：
   - 该项目在头部交易所的覆盖情况（基于实时数据）
   - 3 条推荐/不推荐理由
   - 2 条需要关注的风险点
5. **BYDFi 跟进建议**：高/中/低紧迫性 + 一句理由（对评委的差异化亮点）
6. **最终结论**：绿/黄/红 + 一句话推荐语

---

## 7天开发计划

| 天 | 任务 | 产出 |
|----|------|------|
| Day 1（今天）| 搭框架 + 跑通 CoinGecko 数据拉取（含 tickers 和 developer_data） | 能打印出 BTC 的完整字段 |
| Day 2 | 完成规则评分逻辑（市场/社区/技术3维度），验证 developer_data 覆盖率 | 能跑出5个维度分数 |
| Day 3 | 接入 Claude API，调通分析 prompt | 能输出 Claude 的文字分析 |
| Day 4 | Streamlit 界面 + 雷达图 | 有 UI 可以操作了 |
| Day 5 | 用 5-10 个真实 Token 测试，优化 prompt | 结论准确度提升 |
| Day 6 | 边角打磨：加载状态、错误处理、界面美化 | 路演级别的成品 |
| Day 7（4/6）| 准备演示数据、写 README | 提交 |

---

## 路演脚本（5分钟）

| 环节 | 时长 | 内容 |
|------|------|------|
| 问题定义 | 30s | "上币调研现在需要 2-3 天，我们用 AI 把它压缩到 3 分钟" |
| 现场演示 | 3min | 输入一个 Token → 实时跑出完整报告 |
| 结果解读 | 1min | 对比人工调研，AI 多发现了哪些信号 |
| 收尾 | 30s | "这套系统可以直接接入上币决策流程" |

> 路演时准备 3 个 Token：一个高分、一个中等、一个低分，展示系统的区分度。
