...
def _render_report(result: dict):
    ...
    with col1:
        ...
        st.metric("市值", _fmt_usd(token.get("market_cap_usd")))
        st.metric("24h 交易量", _fmt_usd(token.get("volume_24h_usd")))
        if price:
        ...
        if watchlist:
            st.metric("CoinGecko 关注人数", f"{watchlist:,}")
        if sentiment is not None:
            st.metric("看涨情绪", f"{sentiment:.0f}%")
    ...
    if total >= 75:
        st.success(f"🟢 **强烈推荐上币**　总分 {total} / 100\n\n{summary}")
    elif total >= 55:
        st.warning(f"🟡 **建议观望**　总分 {total} / 100\n\n{summary}")
    else:
        st.error(f"🔴 **不建议上币**　总分 {total} / 100\n\n{summary}")
...
def _run_evaluation(coin: dict):
    ...
    # session_state 缓存，防止重复调用 API
    if cache_key in st.session_state:
        _render_report(st.session_state[cache_key])
        return

    # 数据采集
    with st.spinner(f"📡 采集 {coin['name']} 数据..."):
        try:
    ...
    # Claude 分析
    with st.spinner("🤖 AI 分析中（约 10-20 秒）..."):
        try:
