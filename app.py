...
def _render_report(result: dict):
    ...
    if fallback_reason == "overloaded":
        st.error(
            "🔴 **Anthropic 服务端过载（HTTP 529）** — Claude API 当前过载，"
            "已尝试主模型 + 降级模型多次重试仍失败。\n\n"
            "**当前展示规则评分结果**，AI 文字分析为占位内容。请稍后重新分析。"
        )
    elif fallback_reason == "parse":
        st.warning("⚠️ AI 响应解析失败，已降级到默认结果。请稍后重试或查看日志。")
    elif ai.get("model_degraded"):
        st.warning(
            f"⚠️ 主模型过载，已自动降级到备用模型 `{ai.get('used_model', 'haiku')}` 完成分析。"
            "结果仍然可信，但深度略低于主模型。"
        )
    ...
    with tab_ai:
        ...
        # parse_error 已在顶部用 st.error/warning 突出展示，此处不再重复
